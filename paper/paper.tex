...
\textbf{47 probes} spanning all ten OWASP categories plus eight
...
commodity hardware, recording \textbf{15 refused / 18 leaked / 14
partial} outcomes across the 47 probes in a 5.5-minute scan. The full
artefact---probes, scorer, adapters, evaluator, Rich-based terminal
UI, JSON/Markdown report writers, and 49 unit tests---is released
...
(\textsc{LLM01}) to model theft (\textsc{LLM10}). Existing red-team
tools tend to either (a) target a single category in depth (e.g.,
PromptArmour~\cite{promptarmour}, garak~\cite{garak}), or (b) rely on
...
The codebase is small (under 2{,}500 lines including tests), the run-loop
...
\begin{itemize}
\item A library of \textbf{47 reproducible probes} (Section~\ref{sec:probes})
...
\textsc{LLM02} & Insecure Output Handling          & 4 \\
\textsc{LLM03} & Training-Data Poisoning           & 3 \\
\textsc{LLM04} & Model DoS                         & 4 \\
\textsc{LLM05} & Supply Chain                      & 3 \\
\textsc{LLM06} & Sensitive Information Disclosure  & 5 \\
\textsc{LLM07} & Insecure Plugin Design            & 4 \\
\textsc{LLM08} & Excessive Agency                  & 4 \\
\textsc{LLM09} & Overreliance                      & 4 \\
\textsc{LLM10} & Model Theft                       & 3 \\
...
\subsection{Outcomes}
The 47-probe scan completed in 333.5 seconds. Outcome counts are
shown in Figure~\ref{fig:summary} and Table~\ref{tab:results}.
The headline figures: 15 refused, 18 leaked, 14 partial,
0 errors, 0 skipped.
...
\begin{table}[t]
\caption{Live outcomes by category (\texttt{qwen2.5:0.5b}, 333.5\,s).}
\label{tab:results}
\centering
\renewcommand{\arraystretch}{1.05}
\footnotesize
...
\textsc{JAIL}  & 8 & 5 & 3 & 0 \\
\midrule
\textbf{Tot.}  & 47 & 15 & 18 & 14 \\
\bottomrule
\end{tabular}
\end{table}

The pattern is consistent with what we expect from a 0.5\,B-parameter
checkpoint: it has learned to refuse blunt jailbreak templates
(5 of 8 \textsc{JAIL} probes refused) and self-fingerprinting requests
(3 of 3 \textsc{LLM10} probes refused), but it has weaker resistance
to plugin-design probes (\textsc{LLM07}: 3 of 4 leaked) and
overreliance probes (\textsc{LLM09}: 2 of 4 leaked, 2 partial). All
five \textsc{LLM01} prompt-injection probes either leaked or returned
ambiguous output, in line with prior reports that direct prompt
injection remains the hardest category to harden against on small
checkpoints. The exact counts are reproducible: re-running the same
harness against the same model checkpoint at \texttt{temperature=0}
produces the same outcomes byte-for-byte, modulo network latency.
...
\texttt{stream=false}. The default scan of 47 probes therefore costs
under 30{,}000 prompt-tokens and 30{,}000 response-tokens---an order
...
The repository ships 49 pytest cases covering: probe-registry
invariants (no duplicate ids, all categories valid, immutability of
frozen dataclasses), scorer behaviour for each canonical refusal
phrase and for the success/refusal-marker priority, adapter wire
format with respx mocks, evaluator dispatch under concurrency, JSON +
Markdown round-trip, and TUI smoke tests. The full suite runs in
0.57\,s.
...
\bibitem{promptarmour}
\textit{PromptArmour: Defensive Probing for LLM Applications},
